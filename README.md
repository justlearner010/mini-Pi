...
- **v1.3：已完成。** 交互式会话以低对比度的用户、回答、活动、审批和错误层呈现；工具活动默认折叠，不输出原始事件日志。
...
需要工具授权时，TUI 会用低对比度琥珀色区块显示已过滤的请求详情；敏感操作必须精确输入 `y`，破坏性操作必须精确输入 `yes`。Provider 或 Agent 错误使用红色区块显示安全诊断，并保留可展开的活动记录。
